...
%%API
start_link() ->
    gen_server:start_link({local, other_server}, ?MODULE, [], []).

stop() ->
    gen_server:call(other_server, stop).
add(Value) ->
    gen_server:cast(other_server, {add, Value}).
find(Value) ->
    gen_server:call(other_server, {find, Value}).
delete(Value) ->
    gen_server:cast(other_server, {delete, Value}).
get_all() ->
    gen_server:call(other_server, get_all).
restore_data(Data) ->
    gen_server:cast(other_server, {restore_data, Data}).
save_backup(Backup) ->
    gen_server:cast(other_server, {save_backup, Backup}).
get_backup() ->
    gen_server:call(other_server, get_backup).

%% gen_server callbacks
init([]) ->
    {ok, backup_log:create()}. % Tuple: {Storage from this server, Backup from other server}
...
handle_cast({restore_data, Data}, _State) ->
    % The state is empty, both Data and Backup inside the tuple
    {noreply, Data}.
